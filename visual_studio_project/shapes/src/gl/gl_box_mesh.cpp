...
void box_mesh::create()
{
    ...
    renderer::shader* vertex_shader = new renderer::opengl_shader(vs_desc);

    renderer::shader_desc fs_desc = { renderer::shader_type::fragment, fragment_shader_source };
    renderer::shader* fragment_shader = new renderer::opengl_shader(fs_desc);

    renderer::shader_program_desc sp_desc = { vertex_shader, fragment_shader };
    shader_program = new renderer::opengl_shader_program(sp_desc);

    texture = new renderer::opengl_texture("E:\\dev\\scaling-potato\\bin\\Debug-x64\\platform\\wall.png");
    // texture = new renderer::opengl_texture("\\\\BILL\\Users\\Public\\Public Games\\wall.png");
...
// Buffers
    ...
    vertex_buffer = new renderer::opengl_buffer(vb_desc);

    renderer::buffer_desc ib_desc = { renderer::buffer_type::index, sizeof(index_data), index_data };
    index_buffer = new renderer::opengl_buffer(ib_desc);
...
// Layout
    ...
    layout = new renderer::opengl_layout(l_desc, 3);

    renderer::buffer_desc vs_ub_desc = { renderer::buffer_type::uniform, sizeof(vs_uniform_data), &vs_uniform_data };
    vs_uniform_buffer = new renderer::opengl_buffer(vs_ub_desc);

    renderer::buffer_desc fs_ub_desc = { renderer::buffer_type::uniform, sizeof(fs_uniform_data), &fs_uniform_data };
    fs_uniform_buffer = new renderer::opengl_buffer(fs_ub_desc);


    renderer::opengl_shader_program* sp = static_cast<renderer::opengl_shader_program*>(shader_program);
    sp->bind_uniform_block(0, "VertexUniforms");
    sp->bind_uniform_block(1, "FragmentUniforms");
    sp->bind_sampler(0, "texture_sampler");

    renderer::opengl_buffer* vs_ub = static_cast<renderer::opengl_buffer*>(vs_uniform_buffer);
    vs_ub->bind_uniform_block(0);

    renderer::opengl_buffer* fs_ub = static_cast<renderer::opengl_buffer*>(fs_uniform_buffer);
