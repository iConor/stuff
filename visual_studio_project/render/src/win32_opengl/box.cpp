...
const char* box::fragment_shader_source =
    ...
    "   vec3 color = tint_color * sample.rgb * light_color;\n"
...
// Shader setup
    ...
    // Color
    texture_uniform = glGetUniformLocation(program, "texture_sampler");
...
// Texture setup
    texture_thingie = new render::texture("E:\\dev\\scaling-potato\\bin\\Debug-x64\\platform\\wall.jpg");
    // texture_thingie = new render::texture("\\\\BILL\\Users\\Public\\Public Games\\wall.jpg");
...
// Buffer setup
    ...
    GLint tex_uv_attrib = glGetAttribLocation(program, "tex_uv");
    glEnableVertexAttribArray(vertex_attrib);
    glVertexAttribPointer(vertex_attrib, 3, GL_FLOAT, GL_FALSE, 8 * sizeof(float), (void*)0);
    glEnableVertexAttribArray(normal_attrib);
    glVertexAttribPointer(normal_attrib, 3, GL_FLOAT, GL_FALSE, 8 * sizeof(float), (void*)(3 * sizeof(float)));
    glEnableVertexAttribArray(tex_uv_attrib);
    glVertexAttribPointer(tex_uv_attrib, 2, GL_FLOAT, GL_FALSE, 8 * sizeof(float), (void*)(6 * sizeof(float)));
    // Unbind
...
void box::draw()
{
    ...
    glBindTexture(GL_TEXTURE_2D, texture_thingie->id());
